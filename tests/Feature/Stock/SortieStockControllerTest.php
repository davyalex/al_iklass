...
class SortieStockControllerTest extends TestCase
{
    ...
    public function test_gestionnaire_parc_et_chef_mecanicien_cannot_record_sortie(): void
    {
        $article = Article::factory()->create(['quantite_stock' => 10]);
        $vehicule = Vehicule::factory()->create();

        // Ni le gestionnaire du parc, ni le chef mécanicien (qui passe par une
        // demande de sortie auprès du gestionnaire de stock) ne sortent le stock eux-mêmes.
        foreach (['gestionnaire', 'chef_mecanicien'] as $role) {
